...
/**
 * GUI-07 (shulker box): a shulker box shows all six mod buttons. Its block entity backs a
 * {@code SimpleContainer}-style inventory, so support is decided by the screen-handler allowlists,
 * and {@code ShulkerBoxMenu} is in both the sortable and transferable handler sets.
 */
@ClientGameTest
public class ShulkerBoxButtonsTest implements ClientTest {
  @Override
  public void runTest(ClientTestContext context) {
    try (ClientWorld world = context.worldBuilder().creative().stopTime(true).create()) {
      InvGameTests.openContainer(context, world, Blocks.SHULKER_BOX, ShulkerBoxScreen.class);
      ...
      if (sort != 2 || stack != 2 || transfer != 2 || total != 6) {
        throw new GameTestAssertionException(
            "expected all six buttons (2/2/2) on a shulker box, got "
